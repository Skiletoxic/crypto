...
def load_data(symbol, start_date_str):
    try:
        ...
        status_text.text('📥 Загружаем данные с Yahoo Finance...')
        progress_bar.progress(0.3)
        
        # Загрузка данных с yfinance
        ticker = yf.Ticker(symbol)
        data = ticker.history(start=start_date_str, end=TODAY)
        
        progress_bar.progress(0.7)
        
        if data.empty:
            progress_bar.progress(1.0)
            status_text.text('❌ Данные не найдены!')
            return None
        ...
        # Убираем часовые пояса из даты, если они есть
